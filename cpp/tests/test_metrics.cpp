...
TEST_CASE("Sharpe ratio positive") {
    ...
    double sharpe = qf::compute_sharpe(curve);
    CHECK(sharpe > 0.0);
}

TEST_CASE("Max drawdown zero on uptrend") {
    auto curve = sample_equity_curve();
    double mdd = qf::compute_max_drawdown(curve);
...
TEST_CASE("Win rate basic") {
    ...
    double wr = qf::compute_win_rate(fills);
...
TEST_CASE("Annualized return") {
    ...
    double ann = qf::compute_annualized_return(total, 0, 2ULL * 31'536'000'000'000'000ULL);
    // (1.28)^(1/2) - 1 = ~13.1%
